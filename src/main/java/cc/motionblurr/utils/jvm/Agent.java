package cc.motionblurr.utils.jvm;
...
public class Agent {
    ...
    private static void injectMod() throws Exception {
        ...
        System.out.println("[MotionBlurr Agent] Successfully injected mod: " + jarPath);
    }
    ...
    private static String getCurrentJarPath() {
        try {
            ...
            System.out.println("[MotionBlurr Agent] Detected JAR path: " + absolutePath);

            return absolutePath;
        } catch (Exception e) {
            System.err.println("[MotionBlurr Agent] Error getting JAR path: " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
