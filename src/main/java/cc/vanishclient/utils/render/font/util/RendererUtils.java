package cc.vanishclient.utils.render.font.util;
...
public class RendererUtils {
    ...
    public static void registerBufferedImageTexture(@NonNull Identifier i, @NonNull BufferedImage bi) {
        try {
            ...
            Runnable registration = () -> minecraft.getTextureManager().registerTexture(i, tex);
            if (minecraft.isOnThread()) {
                // Glyph generation happens while rendering. Register immediately so
                // the identifier cannot resolve to the missing-texture atlas during
                // the draw call that requested this glyph page.
                registration.run();
            } else {
                minecraft.execute(registration);
            }
        } catch (Exception e) { // should never happen, but just in case
            throw new IllegalStateException("Unable to register glyph atlas " + i, e);
        }
    }
