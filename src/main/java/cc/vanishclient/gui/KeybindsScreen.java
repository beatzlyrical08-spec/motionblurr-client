...
public class KeybindsScreen extends Screen {
    ...
    private void updateFilteredModules() {
        ...
        for (Module module : VanishClient.INSTANCE.getModuleManager().getModules()) {
            if (query.isEmpty() || module.getDisplayName().toLowerCase().contains(query)) {
    ...
    private void renderBackground() {
        NanoVGRenderer.drawRect(0, 0, width, height, cc.vanishclient.module.modules.client.KeybindsModule.getBackgroundColor());
    }
    ...
    private void renderModuleSearchPopup(int mouseX, int mouseY) {
        ...
        NanoVGRenderer.drawRoundedRectOutline(popupX, popupY, POPUP_WIDTH, POPUP_HEIGHT, 8f, 2f, cc.vanishclient.module.modules.client.KeybindsModule.getAccentColor());
    ...
    private void renderModulePopupHeader(float popupX, float popupY) {
        String title = selectedModule != null ? "Select key for: " + selectedModule.getDisplayName() : "Search Modules";
        float titleSize = 16f;
    ...
    private void renderModuleEntry(Module module, float popupX, float moduleY, int mouseX, int mouseY) {
        ...
        if (isHovered) {
            NanoVGRenderer.drawRoundedRect(popupX + 20, moduleY, POPUP_WIDTH - 40, 30, 4f, cc.vanishclient.module.modules.client.KeybindsModule.getKeyHoverColor());
        }
        
        NanoVGRenderer.drawText(module.getDisplayName(), popupX + 30, moduleY + 8, 11f, TEXT_COLOR);
    ...
    private void renderKey(KeyButton button, boolean isHovered) {
        ...
        if (boundModule != null) {
            NanoVGRenderer.drawRoundedRectOutline(button.x, button.y, button.width, button.height, 4f, 2f, 
                cc.vanishclient.module.modules.client.KeybindsModule.getKeyAssignedColor());
        }
    ...
    private Color getKeyColor(Module boundModule, boolean isHovered, boolean isWaitingForKey) {
        if (isWaitingForKey && isHovered) return cc.vanishclient.module.modules.client.KeybindsModule.getKeySelectedColor();
        if (boundModule != null) {
            if (isHovered) {
                Color assigned = cc.vanishclient.module.modules.client.KeybindsModule.getKeyAssignedColor();
                return new Color(
            ...
            return cc.vanishclient.module.modules.client.KeybindsModule.getKeyAssignedColor();
        }
        if (isHovered) return cc.vanishclient.module.modules.client.KeybindsModule.getKeyHoverColor();
        return cc.vanishclient.module.modules.client.KeybindsModule.getKeyColor();
    }
    ...
    private void renderInstructions() {
        String instruction;
        if (moduleSearchActive) {
            instruction = selectedModule != null ? "Click a key to bind to " + selectedModule.getDisplayName() : "Search and click a module";
        } else if (selectedModule != null) {
            instruction = "Click a key to bind to " + selectedModule.getDisplayName();
        } else {
        ...
        Color instrColor = selectedModule != null ? cc.vanishclient.module.modules.client.KeybindsModule.getKeySelectedColor() : new Color(150, 150, 160, 255);
    ...
    private void assignKeyToModule(Module module, int keyCode) {
        for (Module other : VanishClient.INSTANCE.getModuleManager().getModules()) {
